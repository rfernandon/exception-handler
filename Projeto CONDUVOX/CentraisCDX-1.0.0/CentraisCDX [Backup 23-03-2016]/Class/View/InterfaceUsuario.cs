...
namespace CentraisCDX.Class.View
{
    ...
    {
        private const String VERSION = "1.0.0Beta01-2016";
        ...
        private void InterfaceUsuario_Load(object sender, EventArgs e)
        {
            ...
            // Define o método que ficará monitorando o estado do processo de transferência de dados
        ...
        delegate void estado_processo(estados estado);
        delegate void progresso();
        
        /* --------------------------------------------------------------------------------- */
        /* Funcionalidade : Monitorar os processos de envio e coleta de programação.         */
        /* --------------------------------------------------------------------------------- */
        private void monitor()
        {
            while (true)
            {
                ...
                {
                    if (Processo.countComandoEnviado == 32 && progressBar.Value <= 100)
                    {
                        progresso progress = atualizarBarraProgresso;
                        Invoke(progress);
                        Processo.countComandoEnviado = 0;
                    }
                }

                if (Processo.estado != estados.EXECUTANDO && Processo.estado != estados.PARADO)
                {
                    estado_processo estado = exibirResultadoProcesso;
                    Invoke(estado, Processo.estado);
                    processo.terminarProcesso();
                }
            }
        }

        /* --------------------------------------------------------------------------------- */
        /* Funcionalidade : Atualizar a barra de status. Para cada chamada do método irá a   */
        /*                  barra irá incrementar 1 step                                     */
        /* --------------------------------------------------------------------------------- */
        ...
        /* --------------------------------------------------------------------------------- */
        public void exibirResultadoProcesso(estados estado)
        {
            switch (estado)
            {
                case (estados.CONCLUIDO):
                case (estados.RECEBIDO):
                    MessageBox.Show(Processo.mensagem, "CentralCDX", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    break;

                case (estados.INCORRETO):
                case (estados.INVALIDO):
                case (estados.TIMEOUT):
                case (estados.ABORTADO):
                case (estados.EXCEPTION):
