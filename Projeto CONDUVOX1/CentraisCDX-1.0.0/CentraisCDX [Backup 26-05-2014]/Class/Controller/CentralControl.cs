...
namespace CentraisCDX.Class.Controller
{
    ...
    class CentralControl
    {
        ...
        /* --------------------------------------------------------------------------------- */
        public int verificarConexao(string porta)
        {
            // Lança a exception se a porta COM for inválida
            if (porta == null || !porta.Contains("COM"))
                throw new PortaCOMInvalidaException("A porta COM especificada não é uma porta válida.");

            // Variáveis utilizada no método
            string CMD_START     = "00";
            string CMD_SUB_CHECK = "00";
            string ACK           = "06";
            string NACK          = "15";
            string number        = "00";
            string log           = "VERIFICANDO CONEXÃO:\r\n";
            int result           = 3;

            // Define o CHK
            int _number = this.converterHexToBinary(number);
            int _start  = this.converterHexToBinary(CMD_START);
            int _check  = this.converterHexToBinary(CMD_SUB_CHECK);
            int _chk    = _number ^ (_start ^ _check);
            ...
            TransferDados t = new TransferDados(porta);
            t.abrirPorta();
            t.sendComando(comando);
            ...
            else if (dadosRecebidos == (STX + number + DLE + NACK))
            {
                log += "<-- NACK\r\n";
                result = 1;
            }
        ...
        /*                  Pode lançar 4 exception.                                         */
        /* --------------------------------------------------------------------------------- */

        // DÚVIDA: ACENTRAL PODE ENVIAR ABOUT? EM QUAL SITUAÇÃO?

        public void enviarProgramacaoCentral()
        {
            ArrayList listComandos = new ArrayList();

            string ACK = "06";
            string NACK = "15";
            string log = "ENVIANDO PROGRAMAÇÃO PARA A CENTRAL:\r\n";

            if (this.portaCOM.Contains("COM"))
            {
                try
                {
                    transfer = new TransferDados(this.portaCOM);
        ...
        /* --------------------------------------------------------------------------------- */
        /* Funcionalidade : Grava arquivo de log com os dados trafegados na porta COM.       */
        /* --------------------------------------------------------------------------------- */
        ...
        {
            string CMD_START = "00";
            string CMD_SUB_OUT = "01";

            // Define o número da sequencia
            string number = this.pegarProxNumberSeq();
        ...
        /* --------------------------------------------------------------------------------- */
        /* Funcionalidade : Retorna o comando que aborta o envio da programação.             */
        /* --------------------------------------------------------------------------------- */
        private string getComandoAbortar()
        {
            string CMD_ABORT = "06";
            ...
            // Define o CHK
            int _number = this.converterHexToBinary(number);
            int _abort = this.converterHexToBinary(CMD_ABORT);
            int _chk = _number ^ _abort;
            string chk = _chk.ToString();
            ...
            // Retorna o comando
            return STX + number + CMD_ABORT + DLE + ETX + chk;
        }
        ...
        /* --------------------------------------------------------------------------------- */
        private string pegarProxNumberSeq()
        {
            if (sequencia > 255)
                sequencia = 0;
