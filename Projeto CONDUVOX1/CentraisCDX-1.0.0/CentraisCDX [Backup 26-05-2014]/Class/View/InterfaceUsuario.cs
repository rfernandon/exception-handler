...
namespace CentraisCDX.Class.View
{
    public partial class InterfaceUsuario : Form
    {
        ...
        private enum tela { APRESENTACAO, PROG_INICIAL, LISTA_RAMAL, TRONCO, MESA, ALARME, VIDEO, CONFIGURACAO, NOVO_RAMAL, CARREGANDO };
        ...
        public InterfaceUsuario()
        {
            ...
            // Definindo eventos para tranferência de dados
            CentralControl.ProcessoAbortado += new ProcessoAbortadoEventHandler(transferData_ProcessoAbortado);
            CentralControl.ProcessoConcluido += new ProcessoConcluidoEventHandler(transferData_ProcessoConcluido);
        ...
        private void toolButtonExportarPDF_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("O arquivo já está sendo usado por outro processo. Feche o arquivo e tente novamente.", "Ops! Ocorreu um erro", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            catch (Exception Ex)
            {
                MessageBox.Show(Ex.Message, "Ops! Ocorreu um erro", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void btnTestarConexao_Click(object sender, EventArgs e)
        {
            ...
            {
                this.Cursor = Cursors.WaitCursor;
                switch (cc.verificarConexao((string)this.cboPortaCOM.SelectedItem))
                {
                    case 0:
                        MessageBox.Show("Conexão bem sucedida.", "CentralCDX", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        break;
                    case 1:
                        MessageBox.Show("A central não reconheceu o comando de conexão.", "Ops! Ocorreu um erro", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        break;
                    case 2:
                        MessageBox.Show("A central não respondeu o comando de conexão (timeout).", "Ops! Ocorreu um erro", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        break;
                    case 3:
                        MessageBox.Show("A central enviou uma resposta inválida.", "Ops! Ocorreu um erro", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        break;
                }
            }
            catch (Exception Ex)
        ...
        private void exibirTela(tela id)
        {
            ...
            //this.pnlTelaCarregando.Visible = false;
            ...
            switch ((int)id)
            {
                ...
                case 8:
                    ...
                    break;
                case 9:
                    //this.pnlTelaCarregando.Visible = true;
                    break;
            }
        }
        ...
        /* --------------------------------------------------------------------------------- */
        /* Funcionalidade : Verifica se a central está respondendo.                          */
        /* --------------------------------------------------------------------------------- */
        private void toolButtonEnviar_Click(object sender, EventArgs e)
        {
            try{
                cc.enviarProgramacaoCentral();
            }
            catch (Exception Ex)
            {
                MessageBox.Show(Ex.Message, "Ops! Ocorreu um erro", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void transferData_ProcessoConcluido(object source, EventArgs e)
        {
            MessageBox.Show("Processo Concluido.");
        }

        private void transferData_ProcessoAbortado(object source, EventArgs e)
        {
