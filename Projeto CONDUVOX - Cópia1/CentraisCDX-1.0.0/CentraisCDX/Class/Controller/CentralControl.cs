...
namespace CentraisCDX.Class.Controller
{
    class CentralControl
    {
        ...
        /* --------------------------------------------------------------------------------- */
        public void gravarProgramacaoInicial(ProgramacaoInicial pi)
        {
            central.programacaoInicial = pi;
        }
        ...
        public void definirMesa(MesaOperadora mesa, int id)
        {
            ...
            for (int i = 0; i < 9; i++)
            {
                ...
                if (mesa.buscarPorNomeEstadoTecla(tecla) != estado.DESATIVADA && mesa.buscarPorNomeEstadoTecla(tecla) != estado.FECHADURA)
                {
        ...
        #region OPERAÇÕES DE TRANSMISSÃO DE DADOS
        /* --------------------------------------------------------------------------------- */
        ...
        /* --------------------------------------------------------------------------------- */
        /* Funcionalidade : Inicia o processo que verifica se a central está respondendo na  */
        /*                  porta COM configurada.                                           */
        /* --------------------------------------------------------------------------------- */
        public void verificarConexao(Processo processo, string porta)
        {
            // Inicia o processo
            processo.iniciarProcesso(new Testar(porta));
        }

        /* --------------------------------------------------------------------------------- */
        /* Funcionalidade : Envia a programação para a central pela COM configurada.         */
        /* --------------------------------------------------------------------------------- */
        public void enviarProgramacaoCentral(Processo processo, string porta)
        {
            // Inicia o processo
            processo.iniciarProcesso(new Enviar(central, porta));
        }
        
        /* --------------------------------------------------------------------------------- */
        /* Funcionalidade : Coleta a programação da central pela COM configurada.            */
        /* --------------------------------------------------------------------------------- */
        public void coletarProgramacaoCentral(Processo processo, string porta)
        {
            // Inicia o processo
            processo.iniciarProcesso(new Coletar(central, porta));
        }
        #endregion
    }
}