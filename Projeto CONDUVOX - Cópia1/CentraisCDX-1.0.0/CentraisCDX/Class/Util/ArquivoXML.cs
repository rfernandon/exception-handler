...
namespace CentraisCDX.Class.Util
{
    class ArquivoXML
    {
        ...
        public Central carregarDadosArquivoXML(Object objeto)
        {
            ...
            {
                XmlAttributeCollection id = xn.Attributes;
                formato = id[0].Value.ToString();
            }

            // Verifica se o alvo é um arquivo de programação
            if (formato != "CDX-1.0.0")
            ...
            foreach (XmlNode xn in xmlNodeList)
            {
                pi.qtdeDeRamais = Convert.ToInt32(xn["qtde_ramais_enviar"].InnerText);
        ...
        public void salvarDadosArquivoXML(Central central, String caminho)
        {
            ...
            escritor.WriteElementString("qtde_ramais_enviar", central.programacaoInicial.qtdeDeRamais.ToString());
