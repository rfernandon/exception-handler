...
{
    class Enviar : TransferenciaSaida, IExecutavel
    {
        ...
        public void executar(Processo processo)
        {
            ...
            try
            {
                ...
                montarListaProgramacao();

                // Inicializa o log
                Log.init();
                Log.addText("ENVIANDO A PROGRAMAÇÃO:");

                // Abre a porta COM
                comunicacao.abrirPorta();

                int countComandoEnviado = 0;
                int atualizar = programacao.Count / 100;
                int porcentagem = 0;

                // Envia todos os comandos
                foreach (String c in programacao)
                {
                    String cmd = c;

                    // Atualiza a barra de estatus
                    countComandoEnviado++;
                    if (countComandoEnviado == atualizar)
                    {
                        porcentagem++;
                    ...
                    {
                        abort = true;
                        cmd = retornarComandoDeCancelamento();
                    }
                    ...
                    // Sai do loop caso ocorra algum erro no envio da programação
                    if (respostaTrans != EnumRespostaTrans.ACK)
                    {
                        switch (respostaTrans)
                        {
                            case (EnumRespostaTrans.NACK):
                                processo.mensagem = "A central não conseguiu reconhecer o comando enviado!";
                                processo.estado = EnumEstado.INCORRETO;
                                break;
                            case (EnumRespostaTrans.TIMEOUT):
                                processo.mensagem = "Ocorreu um erro de TIMEOUT ao tentar enviar a programação!";
                                processo.estado = EnumEstado.TIMEOUT;
                                break;
                            case (EnumRespostaTrans.INVALIDO):
                                processo.mensagem = "O aplicativo não conseguiu reconhecer o retorno enviado pela central!";
                    ...
                    // Sai do loop caso o usuário cancele o processo
                    if (abort && respostaTrans == EnumRespostaTrans.ACK)
                    { 
                        processo.mensagem = "O processo foi abortado pelo usuário!";
                        processo.estado   = EnumEstado.ABORTADO;
                        processo.cancelar = false;

                        processo.OnUpdateProgressBar(0);

                        break;
                    }
                }

                // Define o estado de CONCLUIDO caso a resposta for ACK
                if (respostaTrans == EnumRespostaTrans.ACK && !abort)
                {
            ...
            {
                processo.mensagem = "Ocorreu uma falha ao tentar enviar a programação!\n\nErro: " + Ex.Message;
        ...
        private void colherProgramacaoInicial()
        {
            ...
            // nn nn nn nn = FF FF F9 99
            data = "04" + "02" + Convert.ToString(pi.nroPrimeiroBloco).PadLeft(8, 'F');
            addComando(data);
        ...
        { 
            ...
            // Prepara oo alarme
            // A quantidade de ramais que será enviados são definidos na programação inicial
            for (int i = 0; i < central.programacaoInicial.qtdeDeRamais; i++)
            {
                /*
